-- Expose listings.is_under_option in the canonical materialized view so the
-- frontend can show a permanent "Sous option" badge next to the price.
-- The scraper has been writing this column since the delta model round; the
-- back_to_market trigger consumes its transitions, but the current state was
-- not surfaced anywhere. Recreating the matview is cheap at current volume.
